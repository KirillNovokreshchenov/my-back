...
class UsersRepository {
    ...
    async updateConfirm(code: string): Promise<boolean> {
        const result = await EmailConfirmationClass.updateOne({confirmationCode: code}, {$set: {isConfirmed: true}})
        return result.matchedCount === 1
    }
    async updateEmailConfirmationCode(id: ObjectId, newCode: string, date: Date): Promise<boolean> {
        const result = await EmailConfirmationClass.updateOne({userId: id}, {
            $set: {
    ...
    async deleteEmailConfirmation(userId: ObjectId): Promise<boolean> {
        const result = await EmailConfirmationClass.deleteOne({userId: userId})
        return result.deletedCount === 1
    }
    ...
    async newPassword(newPassword: string, email: string): Promise<boolean> {
       const result = await UserModelClass.updateOne({email: email},{$set: {password: newPassword}})
        return result.modifiedCount === 1
